...
Children from the ABCD Study Release 4.0 that have complete pre- and post-injury CBCL information (after imputation for SES variables) will undergo quality control (QC) in this script. This script checked the following: all participants have QC completed, received a passing scor, had RSI data available, were not diagnosed with any neurological conditions.
...
qc_t1_pass %>% 
  group_by(group) %>% 
  summarise(count = n_distinct(subjectkey)) #b = 4061 CTRL, 154 mTBI
```
...
The imaging data used below is from ABCD Data Release 3.0. Regions were labeled using automatic segmentation (subcortical grey matter), Desikan-Killiany atlas (subadjacent WM), and AtlasTrack segmentation (deep white matter). 

```{r echo = FALSE}
## WM, sub-cortical GM, SWM (sub-adjacent WM)
...
## cortical GM, GWC
RSI2 <- read.table('mri_rsi_p202.txt')

rsi2.nd <- select(RSI2, subjectkey, interview_date:sex, eventname, dmri_rsindgm_cdk_bsslh:dmri_rsindgm_cdk_mean, dmri_rsindgwc_cdk_bsslh:dmri_rsindgwc_cdk_mean)
# write.csv(rsi2.nd, "rsi2_nd.csv")
...
length(unique(rsi_missing_nd$subjectkey)) #672 - 680 have missing data
rsi_missing_nd_dups <- rsi_missing_nd %>% 
  group_by(subjectkey) %>% 
  filter(n() == 2) #41 unique - these participants removed because they have missing data for one of the timepoints
rsi_missing_nd_sings <- rsi_missing_nd %>% 
  group_by(subjectkey) %>% 
  filter(n() < 2) #631 - these participants removed because they don't have data for one of the timepoints
...
```{r med history qc, include = FALSE}
